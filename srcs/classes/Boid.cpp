...
Boid::Boid(void)
{
	...
	this->stats = (t_boid){
		(t_lifestats){MAX_HEALTH, MIN_BIRTH_ENERGY, 0.0f, 0, 0, 0, true, false},
		0,
		Vector2Zero(),
		RED,
		50,
	...
	this->stats.color =
		(Color){static_cast<unsigned char>(
					Remap(this->stats.max_steer, 0.1, 0, 0, 255)),
				static_cast<unsigned char>(
					Remap(this->stats.perception, 0, 200, 0, 255)),
				static_cast<unsigned char>(
					Remap(this->stats.max_steer, 0, 0.1, 0, 255)),
				255};
	this->stats.pos = {static_cast<float>(GetRandomValue(0, CANVAS_WIDTH)),
					   static_cast<float>(GetRandomValue(0, CANVAS_HEIGHT))};
	this->vel = {static_cast<float>(GetRandomValue(-this->stats.max_speed,
												   this->stats.max_speed)),
				 static_cast<float>(GetRandomValue(-this->stats.max_speed,
												   this->stats.max_speed))};
...
void Boid::draw_boid(void)
{
	...
	DrawTriangle(
		Vector2Add(this->stats.pos,
				   Vector2Rotate({0, -this->radius * 2.5f}, this->rotation)),
		Vector2Add(this->stats.pos,
				   Vector2Rotate({-this->radius, this->radius * 1.5f},
								 this->rotation)),
		Vector2Add(
			this->stats.pos,
			Vector2Rotate({this->radius, this->radius * 1.5f}, this->rotation)),
		this->stats.color);
}
...
void Boid::draw_align(void)
{
	...
	const Vector2 line =
		Vector2Add(this->stats.pos, this->average.vel * line_length);
...
void Boid::draw_avoid(void)
{
	...
	const Vector2 line =
		Vector2Add(this->stats.pos, this->average.sep * line_length);
...
void Boid::draw_cohese(void)
{
	...
	const Vector2 line =
		Vector2Add(this->stats.pos, this->average.pos * line_length);
...
void Boid::getaverage(Boid *flock)
{
	...
	for (size_t i = 0; i < NB_BOIDS; i++)
	{
		if (this == &flock[i])
			continue;
		if (CheckCollisionCircles(this->stats.pos, this->stats.perception,
								  flock[i].stats.pos, flock[i].radius) == false)
			continue;
		this->average.vel = Vector2Add(this->average.vel, flock[i].vel);
		this->average.pos = Vector2Add(this->average.pos, flock[i].stats.pos);
		total += 1;
		if (CheckCollisionCircles(this->stats.pos,
								  this->stats.perception *
									  this->stats.separation_ratio,
								  flock[i].stats.pos, flock[i].radius) == false)
			continue;
		this->average.sep =
			Vector2Add(this->average.sep,
					   Vector2Subtract(this->stats.pos, flock[i].stats.pos));
		total_avoid += 1;
	}
	if (total <= 0)
		return;
	...
	if (total_avoid <= 0)
		return;
	this->average.sep =
		Vector2Divide(this->average.sep, {total_avoid, total_avoid});
}
...
void Boid::separate(void)
{
	...
	this->average.sep =
		Vector2Scale(this->average.sep, this->stats.max_separation);
...
void Boid::align(void)
{
	...
	this->average.vel =
		Vector2Scale(this->average.vel, this->stats.max_alignment);
	...
	if (length != 0 && length > this->stats.max_steer)
		this->average.vel = Vector2Scale(Vector2Normalize(this->average.vel),
										 this->stats.max_steer);
...
void Boid::cohese(void)
{
	...
	this->average.pos =
		Vector2Scale(this->average.pos, this->stats.max_cohesion);
...
void Boid::runfromborder(void)
{
	...
	if (this->stats.pos.x < this->stats.perception)
		...
		border.x = -RUNBORDER * (this->stats.pos.x -
								 (CANVAS_WIDTH - this->stats.perception));
	if (this->stats.pos.y < this->stats.perception)
		...
		border.y = -RUNBORDER * (this->stats.pos.y -
								 (CANVAS_HEIGHT - this->stats.perception));
...
void Boid::avoidborder(void)
{
	Vector2 border = {0, 0};
	if (this->stats.pos.x < this->stats.perception)
		border.x = this->stats.obstacle_avoidance *
				   (this->stats.perception - this->stats.pos.x);
	else if (this->stats.pos.x > CANVAS_WIDTH - this->stats.perception)
		border.x =
			-this->stats.obstacle_avoidance *
			(this->stats.pos.x - (CANVAS_WIDTH - this->stats.perception));
	if (this->stats.pos.y < this->stats.perception)
		border.y = this->stats.obstacle_avoidance *
				   (this->stats.perception - this->stats.pos.y);
	else if (this->stats.pos.y > CANVAS_HEIGHT - this->stats.perception)
		border.y =
			-this->stats.obstacle_avoidance *
			(this->stats.pos.y - (CANVAS_HEIGHT - this->stats.perception));
	this->acc = Vector2Add(this->acc, border);
}
...
void Boid::attract_towards(Vector2 target)
{
	...
	if (length != 0 && length > this->stats.max_speed_food)
		steer =
			Vector2Scale(Vector2Normalize(steer), this->stats.max_speed_food);
...
void Boid::update(float gamespeed)
{
	...
	this->vel = Vector2Add(this->vel,
						   Vector2Scale(this->acc, GetFrameTime() * gamespeed));
	this->stats.pos = Vector2Add(
		this->stats.pos, Vector2Scale(this->vel, GetFrameTime() * gamespeed));
	this->acc = Vector2Zero();
}
...
bool Boid::lifestatsupdate(int *boids_alive, Boid *boids, float gamespeed)
{
	...
	if (this->stats.life.energy <= 0)
		this->stats.life.health -=
			STARVATION_DAMAGE * GetFrameTime() * gamespeed;
	...
	if (this->stats.life.energy < MAX_ENERGY - foodtoenergy &&
...
t_boid Boid::tweakstats(t_boid stats)
{
	...
	newstats.life = (t_lifestats){
		MAX_HEALTH, MIN_BIRTH_ENERGY, 0.0f, stats.life.generation + 1, 0, 0,
		true,		stats.life.smell};
	newstats.perception = stats.perception + GetRandomValue(-20, 20);
	newstats.perception = Clamp(newstats.perception, 0, MAX_PERCEPTION);
	newstats.max_steer =
		stats.max_steer +
		static_cast<float>(GetRandomValue(-MUTATION_RATE, MUTATION_RATE)) /
			1000.0f;
	newstats.max_steer = Clamp(newstats.max_steer, 0, MAX_ALIGN);
	newstats.max_cohesion =
		stats.max_cohesion +
		static_cast<float>(GetRandomValue(-MUTATION_RATE, MUTATION_RATE)) /
			1000.0f;
	newstats.max_cohesion = Clamp(newstats.max_cohesion, 0, MAX_COHESE);
	newstats.max_separation =
		stats.max_separation +
		static_cast<float>(GetRandomValue(-MUTATION_RATE, MUTATION_RATE)) /
			100.0f;
	newstats.max_separation = Clamp(newstats.max_separation, 0, MAX_SEPARATE);
	newstats.separation_ratio =
		stats.separation_ratio +
		static_cast<float>(GetRandomValue(-MUTATION_RATE, MUTATION_RATE)) /
			100.0f;
	newstats.separation_ratio =
		Clamp(newstats.separation_ratio, 0, MAX_SEPARATION_RATIO);
	newstats.obstacle_avoidance =
		stats.obstacle_avoidance +
		static_cast<float>(GetRandomValue(-MUTATION_RATE, MUTATION_RATE)) /
			1000.0f;
	newstats.obstacle_avoidance =
		Clamp(newstats.obstacle_avoidance, 0, MAX_OBSTACLE_AVOIDANCE);
	if (this->stats.life.smell == true)
		newstats.apetite =
			stats.apetite +
			GetRandomValue(-MUTATION_RATE, MUTATION_RATE) / 100.0f;
	newstats.apetite = Clamp(newstats.apetite, 0, MAX_APETITE);
	if (this->stats.life.smell == true)
		newstats.max_speed_food =
			stats.max_speed_food +
	...
	newstats.color =
		(Color){static_cast<unsigned char>(
					Remap(newstats.max_steer, MAX_ALIGN, 0, 0, 255)),
				static_cast<unsigned char>(
					Remap(newstats.perception, 0, MAX_PERCEPTION, 0, 255)),
				static_cast<unsigned char>(
					Remap(newstats.max_steer, 0, MAX_ALIGN, 0, 255)),
				255};
	return (newstats);
}
