...
void init_engine(t_game *game)
{
	game->player.camera.target =
		(Vector2){CANVAS_WIDTH / 2.0f, CANVAS_HEIGHT / 2.0f};
	...
	game->spawn.circle = {{(float)CANVAS_WIDTH / 2, (float)CANVAS_HEIGHT / 2},
						  (float)CANVAS_HEIGHT / 4};
...
void update_engine(t_game *game)
{
	while (!WindowShouldClose())
	{
		...
		if (game->pause == true)
		{
			DrawText("PAUSED", CANVAS_WIDTH / 2 - 122, CANVAS_HEIGHT / 2, 50,
					 game->textcolor);
			DrawText("Press SPACE to resume", CANVAS_WIDTH / 2 - 110,
					 CANVAS_HEIGHT / 2 + 50, 15, game->textcolor);
			DrawText("Press RIGHT to update by steps", CANVAS_WIDTH / 2 - 140,
					 CANVAS_HEIGHT / 2 + 75, 15, game->textcolor);
		}
		EndDrawing();
	}
}
