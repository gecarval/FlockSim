...
void focus_boid_on_click(t_game *game)
{
	...
	const Vector2 mouseposworld =
		GetScreenToWorld2D(GetMousePosition(), game->player.camera);
	const int mousehash = game->flock.hash.hash(mouseposworld);

	list = game->flock.hash.table[mousehash].boids;
	while (list != NULL)
	{
		if (CheckCollisionCircles(mouseposworld, 5.0f, list->boid->stats.pos,
								  list->boid->radius) == true &&
...
void engine_input_event_handler(t_game *game)
{
	...
	if (game->player.shifting == true)
		game->player.camera.target =
			Vector2Add(game->player.camera.target,
					   Vector2Scale(Vector2Subtract(GetMousePosition(),
													game->player.camera.offset),
									0.05f));
}
...
void spawn_boid(t_game *game)
{
	for (size_t i = 0; i < NB_BOIDS; i++)
	{
		if (game->flock.boids[i].stats.life.alive == false)
		{
			...
			game->flock.boids[i].stats.pos =
				GetScreenToWorld2D(GetMousePosition(), game->player.camera);
			game->flock.boids[i].vel = {
				static_cast<float>(
					GetRandomValue(-game->flock.boids[i].stats.max_speed,
...
void engine_input(t_game *game)
{
	const float walkspeed =
		(0.8f * GetFrameTime()) / (game->player.camera.zoom * 20);
	...
	game->player.camera.zoom +=
		((float)GetMouseWheelMove() * 0.1f * game->player.camera.zoom);
